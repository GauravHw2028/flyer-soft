export type Product={id:string;name:string;pack:string;category:string;price:number;image:string;sku:string};
...
export const templates:Template[]=[
 ...
 {id:'night',name:'Midnight specials',color:'#172554',accent:'#b6ed58',columns:2,capacity:6,style:'night'},
...
export const sampleProducts:Product[]=[
 ['bananas','Fresh bananas','1 kg','Fruit',7.95],['apples','Royal Gala apples','1 kg','Fruit',12.5],['tomatoes','Vine tomatoes','1 kg','Vegetables',8.95],['avocado','Ripe avocados','2 pieces','Fruit',14.95],['oranges','Sweet oranges','1 kg','Fruit',9.95],['broccoli','Fresh broccoli','500 g','Vegetables',10.5]
].map(([id,name,pack,category,price])=>({id:String(id),name:String(name),pack:String(pack),category:String(category),price:Number(price),image:'/products/'+id+'.jpg',sku:''}));
export function newCampaign(brand:Brand,products:Product[]=[],demo=false):Campaign {const start=new Date();const end=new Date(start);end.setDate(end.getDate()+6);return {id:crypto.randomUUID(),name:demo?'Fresh finds · weekly offers':'Untitled campaign',headline:'Fresh picks.\nBig savings.',start:localDate(start),end:localDate(end),template:'fresh',templateSnapshot:{...templates[0],color:brand.color},items:products.map(p=>({...p,offer:Math.round(p.price*.75*100)/100,badge:''})),brand:{...brand},updated:new Date().toISOString(),status:'draft'};}
